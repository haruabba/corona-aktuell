const worldDataSet = [
    ['Land', 'Bestätiger', 'Tod'],
    [{v: 'CN', f: 'China'}, 80696, 3097], 
    [{v: 'KR', f: 'Süd-Korea'}, 7313, 50], 
    [{v: 'IR', f: 'Iran'}, 6566, 194], 
    [{v: 'IT', f: 'Italia'}, 5833, 233], 
    [{v: 'DE', f: 'Deutschland'}, 847, 0], 
    [{v: 'JP', f: 'Japan'}, 461, 6], 
    [{v: 'FR', f: 'Frankreich'}, 949, 16],
    [{v: 'ES', f: 'Spanien'}, 525, 10], 
    [{v: 'SG', f: 'Singapur'}, 138, 0], 
    [{v: 'HK', f: 'Honkong'}, 113, 2], 
    [{v: 'US', f: 'Vereinigte Staaten von Amerika'}, 447, 19],
    [{v: 'KW', f: 'Kuwait'}, 62, 0], 
    [{v: 'TH', f: 'Thailand'}, 50, 1], 
    [{v: 'BH', f: 'Bahrain'}, 85, 0], 
    [{v: 'TW', f: 'Taiwan'}, 45, 1], 
    [{v: 'MY', f: 'Malaysia'}, 99, 0], 
    [{v: 'AU', f: 'Australien'}, 80, 2], 
    [{v: 'AE', f: 'Vereinigte Arabische Emirate'}, 45, 0], 
    [{v: 'GB', f: 'Großbritannien'}, 209, 2], 
    [{v: 'VN', f: 'Vietnam'}, 21, 0], 
    [{v: 'CA', f: 'Kanada'}, 60, 0], 
    [{v: 'MO', f: 'Macao'}, 10, 0], 
    [{v: 'IQ', f: 'Irak'}, 54, 4], 
    [{v: 'SE', f: 'Schweden'}, 161, 0], 
    [{v: 'RU', f: 'Russland'}, 15, 0], 
    [{v: 'CH', f: 'Schweiz'}, 268, 1], 
    [{v: 'AT', f: 'Österreich'}, 104, 0], 
    [{v: 'OM', f: 'Oman'}, 16, 0], 
    [{v: 'NO', f: 'Norwegen'}, 157, 0], 
    [{v: 'IL', f: 'Israel'}, 25, 0], 
    [{v: 'GR', f: 'Griechenland'}, 66, 0], 
    [{v: 'HR', f: 'Kroatien'}, 12, 0], 
    [{v: 'IN', f: 'Indien'}, 39, 0], 
    [{v: 'PH', f: 'Philippinen'}, 7, 1], 
    [{v: 'RO', f: 'Rumänien'}, 13, 0], 
    [{v: 'LB', f: 'Libanon'}, 28, 0], 
    [{v: 'FI', f: 'Finnland'}, 23, 0], 
    [{v: 'PK', f: 'Pakistan'}, 6, 0], 
    [{v: 'NL', f: 'Niederlande'}, 188, 1], 
    [{v: 'NP', f: 'Nepal'}, 1, 0], 
    [{v: 'KH', f: 'Kambodscha'}, 2, 0], 
    [{v: 'LK', f: 'Sri Lanka'}, 1, 0], 
    [{v: 'BE', f: 'Belgien'}, 169, 0], 
    [{v: 'EG', f: 'Ägypten'}, 48, 0], 
    [{v: 'AF', f: 'Afghanistan'}, 4, 1], 
    [{v: 'DZ', f: 'Algerien'}, 19, 0], 
    [{v: 'BR', f: 'Brasilien'}, 19, 0], 
    [{v: 'MK', f: 'Nordmakedonien'}, 3, 0], 
    [{v: 'GE', f: 'Georgia'}, 13, 0], 
    [{v: 'EE', f: 'Estland'}, 10, 0], 
    [{v: 'DK', f: 'Dänemark'}, 27, 0], 
    [{v: 'SM', f: 'San Marino'}, 26, 0], 
    [{v: 'NG', f: 'Nigeria'}, 1, 0], 
    [{v: 'LT', f: 'Litauen'}, 1, 0], 
    [{v: 'NZ', f: 'Neuseeland'}, 5, 0], 
    [{v: 'BY', f: 'Weißrussland'}, 6, 0], 
    [{v: 'MX', f: 'Mexiko'}, 7, 0], 
    [{v: 'AZ', f: 'Aserbaidschan'}, 9, 0], 
    [{v: 'IS', f: 'Island'}, 50, 0], 
    [{v: 'MC', f: 'Monaco'}, 1, 0],
    [{v: 'EC', f: 'Ecuador'}, 15, 0],
    [{v: 'QA', f: 'Katar'}, 12, 0],
    [{v: 'AM', f: 'Armenien'}, 1, 0],
    [{v: 'CZ', f: 'Czech Republic'}, 26, 0],
    [{v: 'PT', f: 'Portugal'}, 25, 0],
    [{v: 'IE', f: 'Irland'}, 19, 0],
    [{v: 'SN', f: 'Senegal'}, 4, 0],
    [{v: 'PS', f: 'Palästina'}, 19, 0],
    [{v: 'CL', f: 'Chile'}, 7, 0],
    [{v: 'ID', f: 'Indonesien'}, 6, 0],
    [{v: 'MA', f: 'Marokko'}, 2, 0],
    [{v: 'SA', f: 'Saudi Arabien'}, 11, 0],
    [{v: 'HU', f: 'Ungarn'}, 5, 0],
    [{v: 'BA', f: 'Bosnien'}, 3, 0],
    [{v: 'LU', f: 'Luxemburg'}, 3, 0],
    [{v: 'DO', f: 'Dominikanische Republik'}, 2, 0],
    [{v: 'JO', f: 'Jordan'}, 1, 0],
    [{v: 'LV', f: 'Lettland'}, 2, 0],
    [{v: 'AD', f: 'Andorra'}, 1, 0],
    [{v: 'TN', f: 'Tunesien'}, 1, 0],
    [{v: 'UA', f: 'Ukraine'}, 1, 0],
    [{v: 'LI', f: 'Liechtenstein'}, 1, 0],
    [{v: 'AR', f: 'Argentinien'}, 9, 0],
    [{v: 'PL', f: 'Polen'}, 6, 0],
    [{v: 'FO', f: 'Färöer Inseln'}, 2, 0],
    [{v: 'SI', f: 'Slowenien'}, 12, 0],
    [{v: 'ZA', f: 'Südafrika'}, 3, 0],
    [{v: 'CR', f: 'Costa Rica'}, 5, 0],
    [{v: 'GI', f: 'Gibraltar'}, 1, 0],
    [{v: 'BT', f: 'Bhutan'}, 1, 0],
    [{v: 'RS', f: 'Republik Serbien'}, 1, 0],
    [{v: 'SK', f: 'Slowakei'}, 3, 0],
    [{v: 'CM', f: 'Kamerun'}, 2, 0],
    [{v: 'VA', f: 'Vatikanland'}, 1, 0],
    [{v: 'MV', f: 'Malediven'}, 2, 0],
    [{v: 'PE', f: 'Peru'}, 6, 0],
    [{v: 'GF', f: 'Französisch-Guayana'}, 5, 0],
    [{v: 'BG', f: 'Bulgarien'}, 4, 0],
    [{v: 'MT', f: 'Malta'}, 3, 0],
    [{v: 'TG', f: 'Togo'}, 1, 0],
    [{v: 'MD', f: 'Maryland'}, 1, 0],
    [{v: 'PY', f: 'Paraguay'}, 1, 0],
  ];

  const germanyDataSet = [
    ['Bundesland', 'Bestätiger', 'Tod'],
    [{v: 'DE-BW', f: 'Baden-Württemberg'}, 182, 0],
    [{v: 'DE-BY', f: 'Bayern'}, 148, 0],
    [{v: 'DE-BE', f: 'Berlin'}, 28, 0],
    [{v: 'DE-BB', f: 'Brandenburg'}, 2, 0],
    [{v: 'DE-HB', f: 'Bremen'}, 4, 0], 
    [{v: 'DE-HH', f: 'Hamburg'}, 13, 0],
    [{v: 'DE-HE', f: 'Hessen'}, 17, 0],
    [{v: 'DE-MV', f: 'Mecklenburg-Vorpommern'}, 7, 0],
    [{v: 'DE-NI', f: 'Niedersachsen'}, 21, 0],
    [{v: 'DE-NW', f: 'Nordrhein-Westfalen'}, 392, 0],
    [{v: 'DE-RP', f: 'Rheinland-Pfalz'}, 15, 0],
    [{v: 'DE-SL', f: 'Saarland'}, 4, 0],
    [{v: 'DE-SN', f: 'Sachsen'}, 4, 0],
    [{v: 'DE-ST', f: 'Sachsen-Anhalt'}, 0, 0],
    [{v: 'DE-SH', f: 'Schleswig-Holstein'}, 8, 0],
    [{v: 'DE-TH', f: 'Thüringen'}, 2, 0]
  ];

    const germanyCitiesDataSet = [
        ['City',   'Population', 'Area'],
        ['Rome',      2761477,    1285.31],
        ['Milan',     1324110,    181.76],
        ['Naples',    959574,     117.27],
        ['Turin',     907563,     130.17],
        ['Palermo',   655875,     158.9],
        ['Genoa',     607906,     243.60],
        ['Bologna',   380181,     140.7],
        ['Florence',  371282,     102.41],
        ['Fiumicino', 67370,      213.44],
        ['Anzio',     52192,      43.43],
        ['Ciampino',  38262,      11]
      ];

    const germanyDataSetProDay = [
        ['X', 'Bestätiger', 'Tod'],
        [new Date(2020, 0, 28), 1, 0],    
        [new Date(2020, 0, 29), 4, 0],   
        [new Date(2020, 0, 30), 4, 0],  
        [new Date(2020, 0, 31), 5, 0],   
        [new Date(2020, 1, 1), 7, 0],  
        [new Date(2020, 1, 2), 8, 0],
        [new Date(2020, 1, 3), 10, 0],    
        [new Date(2020, 1, 4), 12, 0],   
        [new Date(2020, 1, 5), 12, 0],  
        [new Date(2020, 1, 6), 12, 0],   
        [new Date(2020, 1, 7), 13, 0],  
        [new Date(2020, 1, 8), 14, 0],
        [new Date(2020, 1, 9), 14, 0],    
        [new Date(2020, 1, 10), 14, 0],   
        [new Date(2020, 1, 11), 14, 0],  
        [new Date(2020, 1, 12), 16, 0],   
        [new Date(2020, 1, 13), 16, 0],  
        [new Date(2020, 1, 14), 16, 0],
        [new Date(2020, 1, 15), 16, 0],  
        [new Date(2020, 1, 16), 16, 0],
        [new Date(2020, 1, 17), 16, 0],    
        [new Date(2020, 1, 18), 16, 0],   
        [new Date(2020, 1, 19), 16, 0],  
        [new Date(2020, 1, 20), 16, 0],   
        [new Date(2020, 1, 21), 16, 0],  
        [new Date(2020, 1, 22), 16, 0],
        [new Date(2020, 1, 23), 16, 0],    
        [new Date(2020, 1, 24), 16, 0],   
        [new Date(2020, 1, 25), 16, 0],  
        [new Date(2020, 1, 26), 18, 0],   
        [new Date(2020, 1, 27), 21, 0],  
        [new Date(2020, 1, 28), 26, 0],
        [new Date(2020, 1, 29), 57, 0],
        [new Date(2020, 2, 1), 57, 0],  
        [new Date(2020, 2, 2), 129, 0],
        [new Date(2020, 2, 3), 157, 0],    
        [new Date(2020, 2, 4), 196, 0],   
        [new Date(2020, 2, 5), 400, 0],
        [new Date(2020, 2, 6), 639, 0],
        [new Date(2020, 2, 7), 795, 0],
        [new Date(2020, 2, 8), 847, 0],
    ];